...
max = threading.Semaphore(value=50) # Это значение зависит от вашего ПК
threads = []
list = open('proxies.txt', 'r') # Имя файла где хранятся прокси
...
def run(channel, post, proxy):
    ...
    if (type(s) is dict):
        ...
        if l != False: print('Прокси '+proxy+' отработала успешно!')
    max.release()
    print('Прокси поток '+proxy+' был отклонён.')
...
for proxy in proxies:
    ...
    thread.start()
    print('Запущен новый прокси поток: '+p)